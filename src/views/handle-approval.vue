<template>
	<div class="tabbar_bottom">
		<div v-for="(item, index) in fieldOptions" :key="index">
			...
			<!-- 部门 -->
			<van-field
				v-if="!item.hidden && item.type === 'department'"
				input-align="right"
				readonly
				clickable
				v-model="formModel[item.field + '_label']"
				...
				:required="item.required"
			/>
			<van-popup v-model="item.showPicker" position="bottom" v-if="!item.hidden && item.type === 'department'">
				<van-picker
					show-toolbar
					:columns="item.options"
					value-key="label"
			...
			<!-- 成员 -->
			<van-field
				v-if="!item.hidden && item.type === 'member'"
				input-align="right"
				readonly
				clickable
				v-model="formModel[item.field + '_label']"
				...
				:required="item.required"
			/>
			<van-popup v-model="item.showPicker" position="bottom" v-if="!item.hidden && item.type === 'member'">
				<van-picker
					show-toolbar
					:columns="item.options"
					value-key="label"
		...
		<div class="footer van-hairline--top safe-area-bottom" v-if="flowFlag && (flowStatus === 3 || flowStatus === 4)">
			<!-- <div class="more">
				<van-icon name="more-o" size="24" />
				<span>更多</span>
			</div> -->
			<van-button round block type="info" plain @click="handleReject">驳回</van-button>
			<van-button round block type="info" @click="handleAgree">同意</van-button>
		</div>
	</div>
</template>
	...
	methods: {
		async reportDetail() {
			...
			if (res.data.code == 200) {
				...
				this.flowStatus = this.formModel.flowStatus
				...
				// 	} else if (moduleItem.type === 'area') {
				// 		//this.queryRegions(this.moduleData.length, moduleItem)
				// 		let regions = ''
				// 		let areaList = JSON.parse(this.form[moduleItem.table + '.' + moduleItem.field])
				// 		for (let i = 0; i < areaList.length; i++) {
				// 			if (i != areaList.length - 1) {
				// 				regions += this.regions[areaList[i]] + '/'
				// 			} else {
				// 				regions += this.regions[areaList[i]]
				// 			}
				// 		}
				// 		this.form[moduleItem.table + '.' + moduleItem.field + '_label'] = regions
			}
		},
		//单据编号
		...
		async queryDynamic(obj, index) {
			...
			if (res.data.code === 200) {
				...
				for (var i = 0; i < res.data.data.length; i++) {
					...
					if (this.formModel[item.field] == res.data.data[i].id) {
						this.formModel[item.field + '_label'] = res.data.data[i].name
					}
				}
				item.options = options

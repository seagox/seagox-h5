	...
	<div class="tabbar_bottom">
		<div v-for="(item, index) in fieldOptions" :key="index">
			...
			<van-field
				...
				:clearable="item.clearable"
				autocomplete
				@blur="passwordChange($event, item)"
			/>
			<!-- 数字 -->
			<van-field
			...
			/>
			<van-popup v-model="item.showPicker" position="bottom" v-if="item.type === 'select'">
				<van-picker
					show-toolbar
			...
			<van-field
				...
				@click="item.showPicker = true"
			/>
			<van-popup v-model="item.showPicker" position="bottom" v-if="item.type === 'area'">
				<van-picker
					show-toolbar
					:columns="item.options"
					value-key="name"
			...
			<van-field
				...
				@click="item.showPicker = true"
			/>
			<van-popup v-model="item.showPicker" position="bottom" v-if="item.type === 'department'">
				<van-picker
					show-toolbar
					:columns="item.options"
					value-key="label"
			...
			<van-field
				...
				@click="item.showPicker = true"
			/>
			<van-popup v-model="item.showPicker" position="bottom" v-if="item.type === 'member'">
				<van-picker
					show-toolbar
					:columns="item.options"
					value-key="label"
		...
		<div class="footer van-hairline--top safe-area-bottom">
...
export default {
	...
	methods: {
		async queryByMark() {
			const res = await this.$axios.get('jellyForm/queryByMark/' + this.$route.query.formId)
			if (res.data.code == 200) {
				this.flowFlag = res.data.data === 1
			...
			for (let key in this.rules) {
				for (let i = 0; i < this.rules[key].length; i++) {
					...
					if (!this.property[key].hidden) {
						...
						if (item.maxlength && this.formModel[key] && this.formModel[key].length > item.maxlength) {
							this.$toast({
								message: item.message,
								icon: 'info-o'
							})
